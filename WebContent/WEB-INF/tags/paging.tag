	...
	<nav aria-label="Page navigation">
		<ul class="inline-flex font-bold">
				...
				<c:choose>
					<c:when test="${pagedListHolder.page == i}">
						<li class="page-item active"><span
							class="h-10 px-5 text-white transition-colors duration-150 bg-indigo-400 border border-indigo-400 focus:shadow-outline"
							>${i+1}</span></li>
					</c:when>
					<c:otherwise>
						<li><a
			...
			<c:if
				...
				<li class="page-item"><span class="h-10 px-5 text-indigo-400 transition-colors duration-150 bg-white border border-indigo-400 focus:shadow-outline hover:bg-indigo-200">...</span></li>
			</c:if>
			<c:if
